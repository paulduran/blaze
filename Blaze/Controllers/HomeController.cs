...
namespace Blaze.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [AuthActionFilter]
        public ActionResult Search(string account, string url, string auth)
        {
            string fullUrl = string.Format("https://{0}.campfirenow.com/{1}?{2}&format=json", account, url, Request["QUERY_STRING"]);
            var request = (HttpWebRequest)WebRequest.Create(fullUrl);
            request.Method = "GET";
            request.ContentType = "application/json";
            request.Headers["Authorization"] = "Bearer " + auth;
